...
import { EVENT_DND_RENDER_PREVIEW, EVENT_DND_RENDER_PREVIEW_REMOVE_BUTTON } from '@lib/plugin/constants/event-types';
...
export const createRemoveButton = (freeform: Freeform): HTMLElement => {
  ...
  const event = freeform._dispatchEvent(EVENT_DND_RENDER_PREVIEW_REMOVE_BUTTON, {
    button,
...
export const createPreviewContainer: CreatePreviewContainer = (metadata, freeform) => {
  ...
  const event = freeform._dispatchEvent(EVENT_DND_RENDER_PREVIEW, {
    metadata,
    container,
