...
class SummaryService extends Component
{
    public function getSummary(): InstallSummary
    {
        ...
        $settings->customPluginName = !!$settingsService->getPluginName();
        ...
        $settings->includeDefaultFormattingTemplates = !!$settingsService->getSettingsModel()->defaultTemplates;
        $settings->removeNewlinesOnExport = $settingsService->isRemoveNewlines();
        $settings->populateValuesFromGet = !!$settingsService->getSettingsModel()->fillWithGet;
        ...
        $settings->purgeSubmissions = !!$settingsService->getPurgableSpamAgeInDays();
        $settings->purgeInterval = $settingsService->getPurgableSpamAgeInDays();
        $settings->formattingTemplatesPath = !!$settingsService->getSettingsModel()->formTemplateDirectory;
        $settings->sendAlertsOnFailedNotifications = !!$settingsService->getFailedNotificationRecipients();
        $settings->notificationTemplatesPath = !!$settingsService->getSettingsModel()->emailTemplateDirectory;
        ...
        $spam->customHoneypotName = !!$settingsService->getSettingsModel()->customHoneypotName;
        ...
        $spam->purgeSpam = !!$settingsService->getPurgableSpamAgeInDays();
        $spam->purgeInterval = $settingsService->getPurgableSpamAgeInDays();
        $spam->blockEmail = !!$settingsService->getSettingsModel()->blockedEmails;
        $spam->blockKeywords = !!$settingsService->getSettingsModel()->blockedKeywords;
        $spam->blockIp = !!$settingsService->getSettingsModel()->blockedIpAddresses;
        $spam->submissionThrottling = !!$settingsService->getSettingsModel()->submissionThrottlingCount;
        $spam->minSubmitTime = !!$settingsService->getSettingsModel()->minimumSubmitTime;
        $spam->submitExpiration = !!$settingsService->getSettingsModel()->formSubmitExpiration;
        $spam->recaptcha = !!$settingsService->getSettingsModel()->recaptchaEnabled;
        $spam->recaptchaType = $settingsService->getSettingsModel()->recaptchaType;
    ...
    private function isModifiedStatuses(): bool
    {
        ...
        if ($statuses[1] !== 'Pending' || $statuses[2] !== 'Open' || $statuses[3] !== 'Closed') {
            return true;
        }
