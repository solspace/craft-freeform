...
class ConstantContact3 extends MailingListOAuthConnector
{
	...
	/**
	 * @param \Solspace\Freeform\Library\Integrations\MailingLists\DataObjects\ListObject $mailingList
	 * @param array                                                                       $emails
	...
	protected function fetchLists(): array
	{
		...
		$status = $response->getStatusCode();
		if ($status !== 200) {
			$this->getLogger()->error('Could not fetch Constant Contact lists', ['response' => (string) $response->getBody()]);
	...
	/**
	 * @return \Solspace\Freeform\Records\IntegrationRecord
	 * @throws \Solspace\Freeform\Library\Exceptions\Integrations\IntegrationException
	 */
	private function getIntegrationRecord(): IntegrationRecord
	{
		...
		if (!$record) {
			throw new IntegrationException(
				$this->getTranslator()->translate('Mailing List integration with ID {id} not found', ['id' => $this->getId()])
			);
		}
