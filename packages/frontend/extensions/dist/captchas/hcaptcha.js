...
async function ensureHcaptcha() {
    await loadScriptOnce("https://js.hcaptcha.com/1/api.js?render=explicit", "ff-hcaptcha-script");
    if (!window.hcaptcha) {
        throw new Error("hCaptcha failed to initialize.");
    }
    return window.hcaptcha;
}
export function createHcaptchaExtension() {
    ...
    return {
        ...
        async mountCaptcha({ captcha, element }) {
            ...
            const widgetId = api.render(widget, {
                ...
                size: version === "invisible" ? "invisible" : (captcha.size ?? "normal"),
                callback: (value) => {
