...
function supportsPayPal(field) {
    return (field.type === "paypal" ||
        field.frontend?.extension === "payment.paypal" ||
        field.frontend?.renderer === "payment.paypal");
}
function showNotice(root, text, tone = "success") {
    let notice = root.querySelector("[data-ff-paypal-notice]");
    if (!notice) {
...
async function getPayPalSdk(clientId, sandbox, currency) {
...
async function createOrder(orderUrl, integration, values) {
...
async function captureOrder(orderUrl, integration, orderId, values) {
 ...
 */
export function createPayPalPaymentExtension() {
    return {
        name: "payment.paypal",
        ...
        async mount(context) {
            ...
            const paypal = await getPayPalSdk(config.clientId, Boolean(config.sandbox), currency);
            if (!paypal.Buttons) {
            ...
            const buttons = paypal.Buttons({
                ...
                createOrder: async () => {
                    ...
                    const order = await createOrder(orderUrl, config.integration, values);
                ...
                onApprove: async (data) => {
                    ...
                    try {
                        const values = context.getValues?.() ?? {};
                        const capture = await captureOrder(orderUrl, config.integration, orderId, values);
                        if (capture.status !== "COMPLETED") {
                            showNotice(context.element, "PayPal capture failed. Please try again.", "error");
                            return;
                        }
                        context.setValue(orderId);
                        if (context.requestSubmit) {
                            showNotice(context.element, "Payment complete. Your form is being submitted automatically.", "success");
                            await context.requestSubmit();
                            return;
                        }
                        showNotice(context.element, "Payment completed with PayPal. Click Submit to finish.", "success");
                    }
                    catch (error) {
                        showNotice(context.element, error instanceof Error
                            ? error.message
                            : "PayPal capture error. Please try again.", "error");
                    }
                },
                onError: () => {
                    showNotice(context.element, "PayPal reported an error. Please try again.", "error");
                },
            });
