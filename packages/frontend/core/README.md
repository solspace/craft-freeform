...
The manifest uses the `range` renderer and `frontend.config` containing `min`, `max`, and `step`. Its `defaultValue` contains the normalized starting value. The built-in React and Vue adapters render a native range input with a current-value display and bound labels. Import `@solspace/freeform-core/range.css` alongside the headless theme. Classic Twig forms use the bundled Freeform script and stylesheet automatically.
