...
type SquareState = {
  card: SquareCard;
  config: Required<Pick<SquareConfig, "integration" | "paymentUrl">> &
    SquareConfig;
  baseUrl: string;
};
...
const states = new Map<string, SquareState>();

function supportsSquare(field: ManifestFieldDefinition): boolean {
  return (
    field.type === "square" ||
...
async function loadSquareSdk(
  sandbox: boolean,
): Promise<NonNullable<Window["Square"]>> {
  if (window.Square) {
 ...
 */
export function createSquarePaymentExtension(): FreeformExtension {
  return {
    name: "payment.square",
    ...
    async mount(context: FieldMountContext) {
      ...
      const config = (context.field.frontend?.config ?? {}) as SquareConfig;
      if (
      ...
      states.set(context.field.handle, {
        card,
        config: config as SquareState["config"],
    ...
    async beforeSubmit(context: ExtensionSubmitContext) {
      ...
      for (const field of Object.values(context.manifest.fields)) {
        ...
        const response = await fetch(
          resolveUrl(state.baseUrl, state.config.paymentUrl),
          {
            method: "POST",
            credentials: "include",
            headers: {
