...
function supportsStripe(field) {
    return (field.type === "stripe" ||
...
async function readCsrfToken(context) {
    const url = context.manifest.security.csrf?.tokenEndpoint ??
...
async function updatePaymentAmount(state, values) {
    if (!state.amountUrlTemplate || state.amountFields.length === 0) {
        return;
    }
    const amountUrl = state.amountUrlTemplate.replace("{paymentIntentId}", encodeURIComponent(state.paymentIntentId));
    ...
    if (!response.ok) {
        throw new Error(updated.errors?.[0] ??
            updated.message ??
    ...
    if (updated.client_secret) {
        throw new Error("Your payment amount changed. Please re-enter your payment details.");
    }
 ...
 */
export function createStripePaymentExtension() {
    return {
        name: "payment.stripe",
        ...
        async mount(context) {
            ...
            const response = await fetch(resolveUrl(context.baseUrl ?? "", options.intentUrl), {
                method: "POST",
                credentials: "include",
                headers: {
                    Accept: "application/json",
            ...
            const onAmountFieldChange = () => {
                void updatePaymentAmount(state, state.getValues()).catch((error) => {
            ...
            for (const handle of state.amountFields) {
                document
                    .querySelectorAll(`[name="${CSS.escape(handle)}"], [name="${CSS.escape(handle)}[]"]`)
                    .forEach((element) => {
        ...
        async beforeSubmit(context) {
            ...
            for (const field of Object.values(context.manifest.fields)) {
                ...
                const checkpoint = await fetch(resolveUrl(state.baseUrl, state.checkpointUrl), {
                    method: "POST",
                    credentials: "include",
                    headers: {
                        Accept: "application/json",
                ...
                if (!checkpoint.ok) {
                    let message = "Unable to prepare the payment. Please review the form and try again.";
                    try {
                        const body = (await checkpoint.json());
                ...
                if (submitted.error) {
                    ...
                    throw new Error(submitted.error.message ?? "Unable to submit payment details.");
                }
                const returnUrl = new URL("/freeform/payments/stripe/callback", state.baseUrl);
                ...
                if (confirmation.error) {
                    ...
                    throw new Error(confirmation.error.message ?? "Unable to confirm the payment.");
                }
                ...
                if (confirmation.paymentIntent?.id) {
                    returnUrl.searchParams.set("payment_intent", confirmation.paymentIntent.id);
                }
