...
function getConfig(field: ManifestFieldDefinition): FileDndConfig {
  return (field.frontend?.config ?? {}) as FileDndConfig;
}

function supportsFileDnd(field: ManifestFieldDefinition): boolean {
  return (
    field.type === "file-dnd" ||
...
function resolveUploadToken(formHandle: string): string {
...
function resolveBaseUrl(context: FieldMountContext): string {
  if (context.baseUrl) {
  ...
  return {
    ...
    async mount(context: FieldMountContext) {
      ...
      if (Array.isArray(value)) {
        for (const item of value) {
          if (typeof item === "string" && item) {
            ...
            typeof item === "object" &&
            "id" in item &&
            typeof (item as UploadedFileMeta).id === "string"
          ) {
            uploaded.push(item as UploadedFileMeta);
          }
        }
      }
        ...
        for (const file of uploaded) {
          ...
          remove.addEventListener("click", async (event) => {
            ...
            const confirmed = window.confirm(
              config.removeFileMessage || "Are you sure?",
            );
            if (!confirmed) {
              return;
            }
      ...
      const uploadFiles = async (fileList: FileList | File[]) => {
        ...
        for (const file of files) {
          ...
          if (file.size > maxBytes) {
            setMessage(
              `Maximum file upload size is ${Math.round(maxBytes / 1000)}KB`,
            );
            continue;
          }
      ...
      dropzone.addEventListener("click", (event) => {
        if ((event.target as HTMLElement).closest(".ff-file-dnd__remove")) {
          return;
        }
        fileInput.click();
      });
