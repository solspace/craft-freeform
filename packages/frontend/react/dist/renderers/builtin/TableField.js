...
import { canAddTableRow, canRemoveTableRow, emptyTableRow, getTableConfig, normalizeTableRows, resolveTableColumnOptions, } from "@solspace/freeform-core";
...
export function TableFieldRenderer(props) {
    ...
    return (_jsxs("div", { className: props.classNames.input, "data-freeform-table": "", children: [_jsxs("table", { className: "ff-table", children: [_jsx("thead", { children: _jsxs("tr", { children: [columns.map((column, colIndex) => (_jsx("th", { className: column.required ? "is-required" : undefined, "data-column-required": column.required ? "true" : undefined, children: column.label }, `${column.label}-${colIndex}`))), _jsx("th", {})] }) }), _jsx("tbody", { children: rows.map((row, rowIndex) => (_jsxs("tr", { children: [columns.map((column, colIndex) => {
                                    ...
                                    if (column.type === "select" || column.type === "dropdown") {
                                        return (_jsx("td", { children: _jsxs("select", { value: String(cellValue ?? ""), disabled: !enabled, required: column.required || undefined, onChange: (event) => {
                                                ...
                                                }, children: [_jsx("option", { value: "", children: column.placeholder || "Select…" }), optionList.map((option) => (_jsx("option", { value: option.value, children: option.label }, option.value)))] }) }, cellKey));
                                    }
                                    ...
                                    if (column.type === "file") {
                                        const selected = Array.isArray(cellValue)
                                            ? cellValue
                                            : [];
                                        const fileCount = Math.max(1, Number(column.metadata
                                            ?.fileCount ?? 1));
                                        return (_jsxs("td", { children: [_jsx("input", { type: "file", multiple: fileCount > 1, disabled: !enabled, onChange: (event) => {
                                                    ...
                                                    } }), selected.length > 0 ? (_jsx("div", { className: "ff-table__file-names", children: selected.map((file) => (_jsx("span", { children: file.name }, `${file.name}-${file.size}`))) })) : null] }, cellKey));
                                    }
                                    return (_jsx("td", { children: _jsx("input", { type: column.type === "number" ? "number" : "text", value: String(cellValue ?? ""), placeholder: column.placeholder, disabled: !enabled, required: column.required || undefined, onChange: (event) => {
                                                updateCell(rowIndex, colIndex, event.target.value);
                                            } }) }, cellKey));
