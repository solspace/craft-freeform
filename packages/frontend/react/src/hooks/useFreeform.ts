...
export function useFreeform(options: UseFreeformOptions): UseFreeformResult {
  ...
  const getClient = useCallback((): FreeformClient => {
  ...
  const setValue = useCallback(
    (handle: string, value: FieldValue) => {
  ...
  const getValue = useCallback((handle: string) => {
    return formStateRef.current?.getValue(handle);
  }, []);

  const isFieldVisible = useCallback((handle: string) => {
    return formStateRef.current?.isFieldVisible(handle) ?? false;
  }, []);

  const isFieldEnabled = useCallback((handle: string) => {
  ...
  const submit = useCallback(
    async (intent: SubmitIntent = "submit") => {
      const formState = formStateRef.current;
