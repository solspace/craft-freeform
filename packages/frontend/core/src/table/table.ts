...
export function getTableConfig(field: ManifestFieldDefinition): TableConfig {
  return (field.frontend?.config ?? {}) as TableConfig;
}
...
export function resolveInitialRowCount(config: TableConfig): number {
  if (
    config.limitRows === "exact" &&
    config.exactRows &&
    config.exactRows > 0
  ) {
  ...
  if (
    (config.limitRows === "min" || config.limitRows === "range") &&
    config.minRows &&
    config.minRows > 0
  ) {
...
export function canAddTableRow(rows: TableRows, config: TableConfig): boolean {
...
function isCellEmpty(value: TableCellValue, type: string): boolean {
  if (type === "checkbox") {
